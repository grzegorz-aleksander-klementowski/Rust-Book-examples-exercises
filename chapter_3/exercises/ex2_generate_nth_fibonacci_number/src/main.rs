...
fn main() {
    ...
    io::stdin()
        .read_line(&mut fibonaci_numer_wanted)
        .expect("Failed to read line.");
    let fibonaci_numer_wanted = fibonaci_numer_wanted
        .trim()
        .parse::<u64>()
        .expect("E: Invalid type numer.");

    while counter <= fibonaci_numer_wanted {
        if fibonaci_numer_wanted == counter {
            println!("{fibonaci_number}");
        }
